package com.ustc.sharefile.model;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.Paint.Style;
import android.graphics.Point;
import android.util.AttributeSet;
import android.util.TypedValue;
import android.view.Display;
import android.view.View;

public class ClipImageBorderView extends View
{
	/**
	 * 水平方向与View的边距
	 */
	private float mHorizontalPadding;
	/**
	 * 垂直方向与View的边距
	 */
	private float mVerticalPadding;
	/**
	 * 绘制的矩形的宽度
	 */
	private float mWidth;
	/**
	 * 绘制的矩形的宽度
	 */
	private float mHeight;
	/**
	 * 边框的颜色，默认为白色
	 */
	private int mBorderColor = Color.parseColor("#FFFFFF");
	/**
	 * 边框的宽度 单位dp
	 */
	private float mBorderWidth = 1;

	private Paint mPaint;

	public ClipImageBorderView(Context context)
	{
		this(context, null);
	}

	public ClipImageBorderView(Context context, AttributeSet attrs)
	{
		this(context, attrs, 0);
	}

	public ClipImageBorderView(Context context, AttributeSet attrs, int defStyle)
	{
		super(context, attrs, defStyle);
		
	
		mBorderWidth = (float) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, mBorderWidth, getResources()
						.getDisplayMetrics());
		mHorizontalPadding = 460.625f;
		mVerticalPadding = 739.375f;
		mPaint = new Paint();
		mPaint.setAntiAlias(true);
	}
	
	@Override
	protected void onDraw(Canvas canvas)
	{
		super.onDraw(canvas);
		
		
		// 计算矩形区域的宽度
		//mWidth = getmWidth() - 2 * mHorizontalPadding;
		
		// 计算距离屏幕垂直边界 的边距
		mVerticalPadding = (getHeight() - mHeight) / 2.0f;
		mHorizontalPadding = (getWidth() - mWidth) / 2.0f;
		
		mPaint.setColor(Color.parseColor("#aa000000"));
		mPaint.setStyle(Style.FILL);
		// 绘制左边1
		canvas.drawRect(0, 0, mHorizontalPadding, getHeight(), mPaint);
		// 绘制右边2
		canvas.drawRect(mHorizontalPadding + mWidth, 0, getWidth(), getHeight(), mPaint);
		// 绘制上边3
		canvas.drawRect(mHorizontalPadding, 0, mHorizontalPadding + mWidth, mVerticalPadding, mPaint);
		// 绘制下边4
		canvas.drawRect(mHorizontalPadding, getHeight()-mVerticalPadding, mHorizontalPadding + mWidth, getHeight(), mPaint);
		
		// 绘制外边框
		mPaint.setColor(mBorderColor);
		mPaint.setStrokeWidth(mBorderWidth);
		mPaint.setStyle(Style.STROKE);
		
		canvas.drawRect(mHorizontalPadding, mVerticalPadding, getWidth()- mHorizontalPadding, getHeight() - mVerticalPadding, mPaint);

	}

//	@Override
//	protected void onDraw(Canvas canvas)
//	{
//		super.onDraw(canvas);
//		
//		
//		// 计算矩形区域的宽度
//		mWidth = getWidth() - 2 * mHorizontalPadding;
//		
//		// 计算距离屏幕垂直边界 的边距
//		mVerticalPadding = (getHeight() - mWidth) / 2;
//		
//		mPaint.setColor(Color.parseColor("#aa000000"));
//		mPaint.setStyle(Style.FILL);
//		// 绘制左边1
//		canvas.drawRect(0, 0, mHorizontalPadding, getHeight(), mPaint);
//		// 绘制右边2
//		canvas.drawRect(getWidth() - mHorizontalPadding, 0, getWidth(),
//				getHeight(), mPaint);
//		// 绘制上边3
//		canvas.drawRect(mHorizontalPadding, 0, getWidth() - mHorizontalPadding,
//				mVerticalPadding, mPaint);
//		// 绘制下边4
//		canvas.drawRect(mHorizontalPadding, getHeight() - mVerticalPadding,
//				getWidth() - mHorizontalPadding, getHeight(), mPaint);
//		
//		// 绘制外边框
//		mPaint.setColor(mBorderColor);
//		mPaint.setStrokeWidth(mBorderWidth);
//		mPaint.setStyle(Style.STROKE);
//		
//		canvas.drawRect(mHorizontalPadding, mVerticalPadding, getWidth()
//				- mHorizontalPadding, getHeight() - mVerticalPadding, mPaint);
//
//	}

	public void setHorizontalPadding(float mHorizontalPadding)
	{
		this.mHorizontalPadding = mHorizontalPadding;
		
	}
	public void setVerticalPadding(float mVerticalPadding){
		this.mVerticalPadding = mVerticalPadding;
	}



	public float getmHeight() {
		return mHeight;
	}

	public void setmHeight(float height) {
		mHeight = height;
		
	}

	public float getmWidth() {
		return mWidth;
	}

	public void setmWidth(float width) {
		mWidth = width;
	}
	
	

}
